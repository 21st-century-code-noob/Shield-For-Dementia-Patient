...
class JBKenBurnsView: UIView {
    ...
    override func awakeFromNib() {
        ...
        if screenOrientationAwareness {
            ...
            NotificationCenter.default.addObserver(self, selector: #selector(JBKenBurnsView.deviceOrientationDidChange), name: UIDevice.orientationDidChangeNotification, object: nil)
    ...
    @objc internal func deviceOrientationDidChange() {
        ...
        if (UIDeviceOrientation.portrait == newOrientation && !portrait) {
            portrait = true
            didActuallyChange = true
        } else{
            if ((UIDeviceOrientation.landscapeLeft == newOrientation || UIDeviceOrientation.landscapeRight == newOrientation) && portrait){
                portrait = false
                didActuallyChange = true
            }
        }
